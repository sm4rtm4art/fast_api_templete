...
InternalResponse = namedtuple(
    "InternalResponse", 
    ["status_code", "headers", "text", "reason", "content_type", "body"]
)
...
class TestAzureTransportMock:
    ...
    def test_blob_storage_operations(self, azure_config):
        ...
        container_create_req = HttpRequest(
            "PUT", 
            f"https://teststorage.blob.core.windows.net/{test_container_name}?restype=container"
        )
        ...
        mock_responses["create_container"] = HttpResponse(
            request=container_create_req,
            internal_response=container_response
        )
        
        # Mock uploading a blob
        blob_upload_req = HttpRequest(
            "PUT", 
            f"https://teststorage.blob.core.windows.net/{test_container_name}/{test_blob_name}"
        )
        ...
        mock_responses["upload_blob"] = HttpResponse(
            request=blob_upload_req,
            internal_response=upload_response
        )
        ...
        mock_responses["download_blob"] = HttpResponse(
            request=blob_download_req,
            internal_response=download_response
        )
    ...
    def test_servicebus_operations(self, azure_config):
        ...
        send_msg_req = HttpRequest(
            "POST", 
            f"https://test-namespace.servicebus.windows.net/{test_queue_name}/messages"
        )
        ...
        mock_responses["send_message"] = HttpResponse(
            request=send_msg_req,
            internal_response=send_response
        )
        
        # Mock receiving a message
        receive_msg_req = HttpRequest(
            "POST", 
            f"https://test-namespace.servicebus.windows.net/{test_queue_name}/messages/head"
        )
        receive_response = InternalResponse(
            ...
            headers={
                ...
                "BrokerProperties": json.dumps({
                    ...
                    "SequenceNumber": 1,
                    "EnqueuedTimeUtc": "2023-01-01T00:00:00Z"
                })
            },
            text=test_message,
            reason="OK",
        ...
        mock_responses["receive_message"] = HttpResponse(
            request=receive_msg_req,
            internal_response=receive_response
        )
        ...
        with patch.object(service, 'get_queue_client', return_value=servicebus_client):
            ...
            with queue_client.get_queue_receiver(queue_name=test_queue_name) as receiver:
                ...
                receiver._internal_receiver.peek_messages = MagicMock(return_value=[
                    MagicMock(body=test_message.encode())
                ])
    ...
    def test_error_handling(self, azure_config):
        ...
        error_req = HttpRequest(
            "GET", 
            f"https://teststorage.blob.core.windows.net/{test_container_name}"
        )
        ...
        mock_transport.send = MagicMock(return_value=HttpResponse(
            request=error_req,
            internal_response=error_response
        ))
