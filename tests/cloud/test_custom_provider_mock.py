        ...
        self._settings: Dict[str, Any] = {
            "cloud": {
                ...
                "custom": {
                    "name": "local-k8s",
                    "description": (
                        "Local Kubernetes cluster with MinIO, Redis and RabbitMQ"
                    ),
                    "storage": {
                        "type": "minio",
...
class TestCustomProviderMock:
    ...
    def test_redis_operations(self, custom_service: Any, connection_success: bool) -> None:
        ...
        with patch("redis.Redis") as mock_redis_class:
            ...
            with patch.object(
                custom_service, "get_cache_client", return_value=mock_redis
            ):
    ...
    def test_rabbitmq_operations(
        self, custom_service: Any, queue_operation: str, should_succeed: bool
    ) -> None:
                ...
                with patch("pika.BlockingConnection") as mock_conn_class:
                    ...
                    if queue_operation == "declare":
                        if should_succeed:
                            ...
                            mock_channel.queue_declare.side_effect = Exception(
                    ...
                    # We'll patch it to return our mock
                    with patch.object(
                        custom_service, "get_queue_client", return_value=mock_channel
                    ):
