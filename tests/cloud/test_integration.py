...
def custom_settings():
    ...
    class Settings:
        def get(self, key, default=None):
            ...
            if key == "cloud.custom":
                return {
                    "storage": self.get("cloud.custom.storage"),
                    "name": "test-custom"
                }
            return default
...
class TestCloudProviderIntegration:
    ...
    def test_aws_factory_integration(self, mock_resource, mock_client, 
                                      aws_settings, cloud_test_env):
    ...
    def test_hetzner_factory_integration(self, mock_session, 
                                          hetzner_settings, cloud_test_env):
        ...
        mock_session_instance.headers.update.assert_called_once_with({
            'Authorization': f'Bearer {cloud_test_env["HETZNER_API_TOKEN"]}',
            'Content-Type': 'application/json'
        })
    
    @patch('minio.Minio')
    def test_custom_factory_integration(self, mock_minio,
                                         custom_settings, cloud_test_env):
